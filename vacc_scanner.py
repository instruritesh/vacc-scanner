...
"""
DISTRICT_ID = "69"  # Karimganj
DATE = "09-05-2021"     # Double digit format DD-MM-YYYY
MIN_AGE_LIMIT = 18
COWIN_API = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/calendarByDistrict?district_id=" + DISTRICT_ID + "&date=" + DATE

SCAN_TIME_INTERVAL = 60  # 60 seconds
...
MAIL_TO_GMAIL_IDS = ['xxx.yyy@gmail.com', 'ppp.rrr@gmail.com']
...
class CowinScanner():
    ...
    def make_request(cls):
        ...
        try:
            response = requests.get(COWIN_API, headers=my_headers, verify=False)
                ...
                if 'sessions' in center:
                    ...
                    for session in sessions_list:
                        ...
                        if availability > 0 and age_limit >= MIN_AGE_LIMIT:
                            ...
                            notify_dict[center_name] = slot_available
        return notify_dict
    ...
    def format_notif_msg(cls, notify_dict):
        ...
        for key in notify_dict.keys():
            ...
            available = notify_dict[key]
            notif_str = notif_str + center_name + ":" + str(available) + "," + "\n"
        return notif_str

    @classmethod
    def send_notification_mail(cls, notify_dict):
        ...
        message['Subject'] = 'CoWin Vaccine slot availability'  # The subject line
    ...
    def scan(cls):
        ...
        while True:
            ...
            if len(notify_dict.keys()) > 0 and MAIL_NOTIFICATION_ALLOWED:
                cls.send_notification_mail(notify_dict)
